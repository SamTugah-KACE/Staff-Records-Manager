...
class LoginService:
    ...
    async def log_user_in(self, request:Request, response: Response, db: Session = Depends(get_db), form_data: OAuth2PasswordRequestForm = Depends()):
        ...
        if form_data.scopes and "remember_me" in form_data.scopes:
            response.set_cookie(key="RefreshToken", value=refresh_token, httponly=True, secure=is_production, samesite='none', expires=(settings.REMEMBER_ME_REFRESH_TOKEN_IN_MINUTES))
        else:
            response.set_cookie(key="RefreshToken", value=refresh_token, httponly=True, secure=is_production, samesite='none', expires=settings.REFRESH_TOKEN_DURATION_IN_MINUTES)
